...
import { deepEqual } from '@doctormckay/stdlib/objects';
...
import type { TradeOffer } from '../classes/TradeOffer';
...
TradeOfferManager.prototype.doPoll = function (
	...
	this.getOffers(
		...
		(err, sent, received) => {
			...
			const origPollData = JSON.parse(JSON.stringify(this.pollData)) as typeof this.pollData;
			...
			for (const offer of (sent ?? [])) {
				if (!sentOffers[offer.id!]) {
					if (!this._pendingOfferSendResponses) {
						if (offer.fromRealTimeTrade) {
							if (
								...
								(offer.state === ETradeOfferState.Active && offer.confirmationMethod !== EConfirmationMethod.None)
							) {
					...
					if (!offer.isGlitched()) {
						...
						this.emit(
							'debug',
							`Not emitting sentOfferChanged for ${offer.id} right now because it's glitched ` +
				...
				if (offer.state === ETradeOfferState.Active) {
					...
					const customCancelTime = offer.data('cancelTime') as number | undefined;
					if (customCancelTime !== undefined) cancelTime = customCancelTime;

					if (cancelTime && Date.now() - offer.updated!.getTime() >= cancelTime) {
						offer.cancel((err) => {
							if (!err) this.emit('sentOfferCanceled', offer, 'cancelTime');
							else this.emit('debug', `Can't auto-cancel offer #${offer.id}: ${err.message}`);
						});
					}
				}
				...
				if (offer.state === ETradeOfferState.CreatedNeedsConfirmation && this.pendingCancelTime) {
					...
					const customPCT = offer.data('pendingCancelTime') as number | undefined;
					if (customPCT !== undefined) pendingCancelTime = customPCT;

					if (pendingCancelTime && Date.now() - offer.created!.getTime() >= pendingCancelTime) {
						offer.cancel((err) => {
							if (!err) this.emit('sentPendingOfferCanceled', offer);
							else this.emit('debug', `Can't auto-cancel pending offer #${offer.id}: ${err.message}`);
						});
					}
				}
			}
			...
			if (this.cancelOfferCount) {
				const sentActive = (sent ?? []).filter((o) => o.state === ETradeOfferState.Active);
				if (sentActive.length >= this.cancelOfferCount) {
					let oldest: TradeOffer = sentActive[0]!;
					...
					if (Date.now() - oldest.updated!.getTime() >= this.cancelOfferCountMinAge) {
						oldest.cancel((err) => {
							if (!err) this.emit('sentOfferCanceled', oldest, 'cancelOfferCount');
						});
					}
				}
			}
			...
			for (const offer of (received ?? [])) {
				if (offer.isGlitched()) { hasGlitchedOffer = true; continue; }

				if (offer.fromRealTimeTrade) {
					if (
						!receivedOffers[offer.id!] &&
						(
							offer.state === ETradeOfferState.CreatedNeedsConfirmation ||
							(offer.state === ETradeOfferState.Active && offer.confirmationMethod !== EConfirmationMethod.None)
						)
				...
				if (!receivedOffers[offer.id!] && offer.state === ETradeOfferState.Active) {
