...
function TradeOfferManager(options) {
	...
	if (options.dataDirectory) {
		this._dataDirectory = options.dataDirectory;
		FS.mkdir(options.dataDirectory, { recursive: true }).catch(() => {
		});
	}
...
TradeOfferManager.prototype._persistToDisk = function(filename, content) {
	...
	var filePath = Path.join(this._dataDirectory, filename);
	...
	if (this._dataGzip) {
		Zlib.gzip(content, (err, data) => {
			if (err) {
				...
				FS.writeFile(filePath + '.gz', data).catch((err) => {
...
TradeOfferManager.prototype._getFromDisk = function(filenames, callback) {
	...
	// Read all files in parallel using native fs.promises
	var readPromises = filenames.map((filename) => {
		var filePath = Path.join(this._dataDirectory, filename);
		return FS.readFile(filePath).then((contents) => {
			return { filename, contents };
		}).catch(() => {
			// File doesn't exist or can't be read - return null (matches old behavior)
			return null;
		});
	});
	...
	Promise.all(readPromises).then((results) => {
		...
		keys.forEach((filename) => {
			Zlib.gunzip(files[filename], (gunzipErr, data) => {
				// Match previous behavior: on error, treat this file as null/absent
				if (!gunzipErr) {
				...
				if (pending === 0) {
					var renamed = {};
					for (var i in gunzipped) {
						if (gunzipped.hasOwnProperty(i)) {
							renamed[i.replace(/\.gz$/, '')] = gunzipped[i];
						}
					}
...
TradeOfferManager.prototype.createOffer = function(partner, token) {
	if (typeof partner === 'string' && partner.match(/^https?:\/\//)) {
		...
		var url = new URL(partner);
		var partnerParam = url.searchParams.get('partner');
